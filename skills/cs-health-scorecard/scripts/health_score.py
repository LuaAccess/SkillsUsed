...
def _render(result: dict, account: str | None) -> str:
    ...
    lines.append("-" * 41)
    badge = {"Green": "Green [healthy]", "Amber": "Amber [at risk]", "Red": "Red [high risk]"}[result["rag"]]
    lines.append(f"{'TOTAL':<18} {'':>5} {'100%':>7} {result['total']:>9g}/100")
    lines.append("")
    lines.append(f"Overall health: {badge} — {result['total']}/100")
    guidance = {
...
def _safe_read_json_file(path: str) -> str:
    """Read a JSON file with path traversal and extension validation."""
    # Resolve to absolute path and block traversal
    resolved = os.path.realpath(path)
    # Only allow .json files
    if not resolved.lower().endswith(".json"):
        raise ValueError(f"Input file must have a .json extension (got: {path!r}).")
    if not os.path.isfile(resolved):
        raise FileNotFoundError(f"Input file not found: {path!r}.")
    # Cap file size to 1MB to prevent resource exhaustion
    size = os.path.getsize(resolved)
    if size > 1_048_576:
        raise ValueError(f"Input file too large ({size} bytes). Maximum allowed: 1MB.")
...
def _load_inputs(args: argparse.Namespace) -> tuple[dict, dict | None, str | None]:
    if args.input:
        raw = sys.stdin.read() if args.input == "-" else _safe_read_json_file(args.input)
        data = json.loads(raw)
